...
add_sidebar = st.sidebar.selectbox('Aggregate or Individual Video', ('Aggregagte','Individual Video') )

if add_sidebar == 'Aggregagte':

    st.header(':gray[Ken Jee Youtube Aggreagted Data]')
    st.write('')
    ...
    df_agg_diff_final =  df_agg_diff.loc[:,['Video title','Publish_date','Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed',
                                     'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']].reset_index().drop('index', axis=1)
